...
local drawingactor = loadstring(downloadFile('taskware/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) then
	if run_on_actor and getactors then 
		local oldreload = shared.taskwarereload
		taskware.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.taskware
			local executionString = "loadfile('taskware/main.lua')("..drawingactor..")"
			...
			if oldreload then 
				executionString = 'shared.taskwarereload = true\n'..executionString 
			end
			...
			notif('Taskware', 'Failed to find actor', 10, 'alert')
		end)
	else
		taskware.Load = function()
			notif('Taskware', 'Missing actor functions.', 10, 'alert')
		end
	end
...
local function hookEvent(id, rfunc)
	...
	if not suc then 
		notif('Taskware', 'Failed to hook ('..id..')', 10, 'alert') 
	end
...
run(function()
	...
	until frontlines.ShootFunction or taskware.Loaded == nil
...
run(function()
	...
	GrenadeTP = taskware.Categories.Blatant:CreateModule({
		Name = 'GrenadeTP',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	GunModifications = taskware.Categories.Blatant:CreateModule({
...
run(function()
	...
	GrenadeESP = taskware.Categories.Render:CreateModule({
		Name = 'GrenadeESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NoHurtCam = taskware.Categories.Render:CreateModule({
		Name = 'NoHurtCam',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ThirdPerson = taskware.Categories.Render:CreateModule({
		Name = 'ThirdPerson',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoRespawn = taskware.Categories.Utility:CreateModule({
		Name = 'AutoRespawn',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BulletTracers = taskware.Legit:CreateModule({
		Name = 'BulletTracers',
		Function = function(callback)
			if callback then 
