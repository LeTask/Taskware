local loadstring = function(...)
	...
	if err and taskware then taskware:CreateNotification('Taskware', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/LeTask/Taskware/'..readfile('taskware/profiles/commit.txt')..'/'..select(1, path:gsub('taskware/', '')), true) end)
		if not suc or res == '404: Not Found' then error(res) end
		if path:find('.lua') then res = '--This watermark is used to delete the file if its cached, remove it to make the file persist after taskware updates.\n'..res end
		writefile(path, res)
	end
...
local vm = loadstring(downloadFile('taskware/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
		repeat task.wait() until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or taskware.Loaded == nil
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Taskware', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
...
run(function()
	...
	Wallbang = taskware.Categories.Combat:CreateModule({
		Name = 'Wallbang',
		Function = function(callback)
			if callback then
				local hook
...
run(function()
	...
	AutoArrest = taskware.Categories.Blatant:CreateModule({
		Name = 'AutoArrest',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoPop = taskware.Categories.Blatant:CreateModule({
		Name = 'AutoPop',
		Function = function(callback)
			if callback then
				task.spawn(function()
...
run(function()
	...
	Punch = taskware.Categories.Blatant:CreateModule({
		Name = 'AutoPunch',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTaze = taskware.Categories.Blatant:CreateModule({
		Name = 'AutoTaze',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	LazerGodmode = taskware.Categories.Blatant:CreateModule({Name = 'LazerGodmode'})
end)
...
run(function()
	...
	InfiniteNitro = taskware.Categories.Utility:CreateModule({
		Name = 'InfiniteNitro',
		Function = function(callback)
			if callback then
