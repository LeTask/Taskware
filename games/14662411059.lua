...
taskware.Place = 11630038968
if isfile('taskware/games/'..taskware.Place..'.lua') then
	loadstring(readfile('taskware/games/'..taskware.Place..'.lua'), 'bridge duel')()
else
	if not shared.TaskDeveloper then
		local suc, res = pcall(function() return 
			game:HttpGet('https://raw.githubusercontent.com/LeTask/Taskware/'..readfile('taskware/profiles/commit.txt')..'/games/'..taskware.Place..'.lua', true) 
		end)
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('taskware/games/'..taskware.Place..'.lua'), 'bridge duel')()
		end
	end
end