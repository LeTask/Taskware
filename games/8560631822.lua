...
taskware.Place = 6872274481
if isfile('taskware/games/'..taskware.Place..'.lua') then
	loadstring(readfile('taskware/games/'..taskware.Place..'.lua'), 'bedwars')()
else
	if not shared.TaskDeveloper then
		...
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('taskware/games/'..taskware.Place..'.lua'), 'bedwars')()
		end
	end
end