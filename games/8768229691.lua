...
run(function()
	...
	local function updateStore(newStore, oldStore)
		if newStore.GameCurrency ~= oldStore.GameCurrency then
			taskEvents.CurrencyChange:Fire(table.clone(newStore.GameCurrency.Quantities))
		end
	...
	taskware:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos)
			store.blocks[pos] = v
		end)
	end))
	taskware:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos)
...
run(function()
	...
	InvMove = taskware.Categories.Blatant:CreateModule({
		Name = 'InvMove',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local anims, armC0 = taskware.Libraries.auraanims
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					if Attacking and taskware.ThreadFix then
						setthreadidentity(8)
					end
...
run(function()
	...
	NoFall = taskware.Categories.Blatant:CreateModule({
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoBuy = taskware.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoBuy:Clean(taskEvents.CurrencyChange.Event:Connect(buyCheck))
	...
	Viewmodel = taskware.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then 
				...
				taskware:Clean(ViewmodelMotor)
				taskware:Clean(runService.RenderStepped:Connect(function()
				...
				taskware:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
				if entitylib.isAlive then 
