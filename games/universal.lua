...
local tween = taskware.Libraries.tween
local targetinfo = taskware.Libraries.targetinfo
local getfontsize = taskware.Libraries.getfontsize
local getcustomasset = taskware.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('taskware/assets/new/blur.png')
...
local function isFriend(plr, recolor)
	if taskware.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(taskware.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and taskware.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(taskware.Categories.Targets.ListEnabled, plr.Name) and true
end
...
local function canClick()
	...
	return (not taskware.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function notif(...)
	return taskware:CreateNotification(...)
end
...
local function serverHop(pointer, filter)
	visited = shared.taskwareserverhoplist and shared.taskwareserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('Taskware', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Taskware', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Taskware', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Taskware', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end

taskware:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.taskwareserverhoplist = '"..table.concat(visited, '/').."'\nshared.taskwareserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('taskware/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('taskware/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('taskware/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
taskware.Libraries.entity = entitylib
taskware.Libraries.whitelist = whitelist
taskware.Libraries.prediction = prediction
taskware.Libraries.hash = hash
taskware.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if taskware.Categories.Main.Options['Teams by server'].Enabled then
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and taskware.Categories.Main.Options['Use team color'].Enabled) then return end
		if isFriend(ent, true) then
			return Color3.fromHSV(taskware.Categories.Friends.Options['Friends color'].Hue, taskware.Categories.Friends.Options['Friends color'].Sat, taskware.Categories.Friends.Options['Friends color'].Value)
		end
		return tostring(ent.TeamColor) ~= 'White' and ent.TeamColor.Color or nil
	end

	taskware:Clean(function()
		entitylib.kill()
		entitylib = nil
	end)
	taskware:Clean(taskware.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	taskware:Clean(taskware.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	taskware:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	taskware:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = taskware.Uninject
				taskware.Uninject = function()
					notif('Taskware', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('Taskware', plr.Name..' is using vape!', 60)
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				taskware:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				taskware:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('taskware/profiles/whitelist.json') and readfile('taskware/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				taskware:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and taskware.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('taskware/profiles/whitelist.json', whitelist.textdata)
				end)
			end
	...
	whitelist.commands = {
			...
			task.spawn(function()
				...
				task.delay(0.6, function()
					local modPrompt = Roact.createElement(auth, {
						style = {},
						screenSize = taskware.gui.AbsoluteSize or Vector2.new(1920, 1080),
						moderationDetails = {
		...
		toggle = function(args)
			...
			if args[1]:lower() == 'all' then
				for i, v in taskware.Modules do
...
run(function()
	...
	AimAssist = taskware.Categories.Combat:CreateModule({
		Name = 'AimAssist',
		Function = function(callback)
			...
			if callback then 
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not taskware.gui.ScaledGui.ClickGui.Visible then
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Position = taskware.gui.AbsoluteSize / 2
...
run(function()
	...
	AutoClicker = taskware.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not taskware.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	Reach = taskware.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SilentAim = taskware.Categories.Combat:CreateModule({
		Name = 'SilentAim',
		Function = function(callback)
			if CircleObject then
...
run(function()
	...
	TriggerBot = taskware.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AntiFall = taskware.Categories.Blatant:CreateModule({
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Fly = taskware.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			if Platform then
...
run(function()
	...
	HighJump = taskware.Categories.Blatant:CreateModule({
		Name = 'HighJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitBoxes = taskware.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Invisible = taskware.Categories.Blatant:CreateModule({
		Name = 'Invisible',
		Function = function(callback)
			if callback then
				if not proper then
...
run(function()
	...
	Killaura = taskware.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	LongJump = taskware.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MouseTP = taskware.Categories.Blatant:CreateModule({
		Name = 'MouseTP',
		Function = function(callback)
			if callback then
				local position
...
run(function()
	...
	Phase = taskware.Categories.Blatant:CreateModule({
		Name = 'Phase',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speed = taskware.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	Spider = taskware.Categories.Blatant:CreateModule({
		Name = 'Spider',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SpinBot = taskware.Categories.Blatant:CreateModule({
		Name = 'SpinBot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Swim = taskware.Categories.Blatant:CreateModule({
		Name = 'Swim',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TargetStrafe = taskware.Categories.Blatant:CreateModule({
		Name = 'TargetStrafe',
		Function = function(callback)
			if callback then
				if not module then
				...
				local flymod, ang, oldent = taskware.Modules.Fly or {Enabled = false}
...
run(function()
	...
	Timer = taskware.Categories.Blatant:CreateModule({
		Name = 'Timer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function Added(ent)
		...
		EntityArrow.Image = getcustomasset('taskware/assets/new/arrowmodule.png')
	...
	Arrows = taskware.Categories.Render:CreateModule({
		Name = 'Arrows',
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(taskware.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Chams = taskware.Categories.Render:CreateModule({
		Name = 'Chams',
		Function = function(callback)
			if callback then
				...
				Chams:Clean(taskware.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	ESP = taskware.Categories.Render:CreateModule({
		Name = 'ESP',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(taskware.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = taskware.Categories.Render:CreateModule({
		Name = 'GamingChair',
		Function = function(callback)
			if callback then
				if taskware.ThreadFix then
				...
				--movingsound.SoundId = downloadVapeAsset('taskware/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadVapeAsset('taskware/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = taskware.Modules.Fly and taskware.Modules.Fly.Enabled or taskware.Modules.LongJump and taskware.Modules.LongJump.Enabled or taskware.Modules.InfiniteFly and taskware.Modules.InfiniteFly.Enabled
...
run(function()
	...
	Health = taskware.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = taskware.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(taskware.Categories.Friends.ColorUpdate.Event:Connect(function()
						ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
					end))
				end
...
run(function()
	...
	PlayerModel = taskware.Categories.Render:CreateModule({
		Name = 'PlayerModel',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	Radar = taskware:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('taskware/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(taskware.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Search = taskware.Categories.Render:CreateModule({
		Name = 'Search',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SessionInfo = taskware:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.taskwaresessioninfo = '"..httpService:JSONEncode(taskware.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.taskwaresessioninfo then
					for i, v in httpService:JSONDecode(shared.taskwaresessioninfo) do
						if taskware.Libraries.sessioninfo.Objects[i] and v.Saved then
							taskware.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end
	
				repeat
					if taskware.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end
						for i, v in taskware.Libraries.sessioninfo.Objects do
	...
	taskware:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if taskware.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (taskware.gui.AbsoluteSize.X / 2)
	...
	addBlur(infoholder)
	taskware.Libraries.sessioninfo = {
		Objects = {},
	...
	taskware.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
run(function()
	...
	local function Loop()
		local screenSize = taskware.gui.AbsoluteSize
	...
	Tracers = taskware.Categories.Render:CreateModule({
		Name = 'Tracers',
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(taskware.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	WaypointFolder.Parent = taskware.gui
	
	Waypoints = taskware.Categories.Render:CreateModule({
		Name = 'Waypoints',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AnimationPlayer = taskware.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = taskware.Categories.Utility:CreateModule({
		Name = 'AntiRagdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = taskware.Categories.Utility:CreateModule({
		Name = 'AutoRejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = taskware.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	ChatSpammer = taskware.Categories.Utility:CreateModule({
		Name = 'ChatSpammer',
		Function = function(callback)
			if callback then
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	Disabler = taskware.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	taskware.Categories.Utility:CreateModule({
		Name = 'Panic',
		Function = function(callback)
			if callback then
				for _, v in taskware.Modules do
					if v.Enabled then
						v:Toggle()
					end
				end
...
run(function()
	...
	Rejoin = taskware.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = taskware.Categories.Utility:CreateModule({
		Name = 'ServerHop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.taskwareserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.taskwareserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.taskwareserverhopprevious)
			end
		end
	})
end)
...
run(function()
	...
	local function playerAdded(plr)
		if not taskware.Loaded then 
			repeat task.wait() until taskware.Loaded 
		end
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				taskware.Save = function() end
				if taskware.Profile ~= Profile.Value then
					taskware.Profile = Profile.Value
					taskware:Load(true, Profile.Value)
				end
	...
	StaffDetector = taskware.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	taskware.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Freecam = taskware.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = taskware.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = taskware.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then 
				local oldfloor
...
run(function()
	...
	Xray = taskware.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MurderMystery = taskware.Categories.Minigames:CreateModule({
		Name = 'MurderMystery',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = taskware.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = taskware.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Cape = taskware.Legit:CreateModule({
		Name = 'Cape',
		Function = function(callback)
			if callback then
				part = Instance.new('Part')
...
run(function()
	...
	ChinaHat = taskware.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if taskware.ThreadFix then
...
run(function()
	...
	Clock = taskware.Legit:CreateModule({
		Name = 'Clock',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Disguise = taskware.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FOV = taskware.Legit:CreateModule({
		Name = 'FOV',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FPS = taskware.Legit:CreateModule({
		Name = 'FPS',
		Function = function(callback)
			if callback then
				local frames = {}
...
run(function()
	...
	Keystrokes = taskware.Legit:CreateModule({
		Name = 'Keystrokes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = taskware.Legit:CreateModule({
		Name = 'Memory',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Ping = taskware.Legit:CreateModule({
		Name = 'Ping',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = taskware.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speedmeter = taskware.Legit:CreateModule({
		Name = 'Speedmeter',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TimeChanger = taskware.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then
