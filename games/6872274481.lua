...
local entitylib = taskware.Libraries.entity
local targetinfo = taskware.Libraries.targetinfo
local sessioninfo = taskware.Libraries.sessioninfo
local uipallet = taskware.Libraries.uipallet
local tween = taskware.Libraries.tween
local color = taskware.Libraries.color
local whitelist = taskware.Libraries.whitelist
local prediction = taskware.Libraries.prediction
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		taskEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						taskEvents.AttributeChanged:Fire(attr)
					end))
				else
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Taskware', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
			...
			return {
				...
				SendToServer = function(_, knockback)
					...
					if damage.knockbackId and (not damage.knockbackMultiplier or not damage.knockbackMultiplier.disabled) then
						...
						taskEvents.KnockbackReceived:Fire()
					end
				end
			}
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				taskEvents.InventoryChanged:Fire()
			end

			if newinv.inventory.items ~= oldinv.inventory.items then
				taskEvents.InventoryAmountChanged:Fire()
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'EntityDamageEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not taskware.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			taskware:Clean(connection:Connect(function(...)
				taskEvents[event]:Fire(...)
			end))
		end)
	end
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		bedwars.ClientDamageBlock:WaitFor(event):andThen(function(connection)
			if not taskware.Connections then return end
			taskware:Clean(connection:Connect(function(data)
				for i, v in cache do
				...
				taskEvents[event]:Fire(data)
			end))
		end)
	end

	taskware:Clean(taskEvents.KnockbackReceived.Event:Connect(function()
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or taskware.Loaded == nil
			if taskware.Loaded == nil then return end
			mapname = workspace:WaitForChild('Map', 5):WaitForChild('Worlds', 5):GetChildren()[1].Name
			mapname = string.gsub(string.split(mapname, '_')[2] or mapname, '-', '') or 'Blank'
		end)
	end)

	taskware:Clean(taskEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	taskware:Clean(taskEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	taskware:Clean(taskEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until taskware.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
...
run(function()
	...
	taskware.Categories.Combat:CreateModule({
		Name = 'NoClickDelay',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Sprint = taskware.Categories.Combat:CreateModule({
		Name = 'Sprint',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then 
...
run(function()
	...
	Velocity = taskware.Categories.Combat:CreateModule({
		Name = 'Velocity',
		Function = function(callback)
			if callback then
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if taskware.Modules.Fly.Enabled or taskware.Modules.InfiniteFly.Enabled or taskware.Modules.LongJump.Enabled then
	...
	Fly = taskware.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(taskEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	InfiniteFly = taskware.Categories.Blatant:CreateModule({
		Name = 'InfiniteFly',
		Function = function(callback)
			frictionTable.InfiniteFly = callback or nil
			updateVelocity()
			if callback then
				if taskware.Modules.Invisible and taskware.Modules.Invisible.Enabled then
					taskware.Modules.Invisible:Toggle()
...
run(function()
	taskware.Categories.Blatant:CreateModule({
		Name = 'KeepSprint',
		Function = function(callback)
...
run(function()
	...
	local anims, AnimDelay, AnimTween, armC0 = taskware.Libraries.auraanims, tick()
...
run(function()
	...
	local LongJumpMethods = {
		...
		cat = function()
			LongJump:Clean(taskEvents.CatPounce.Event:Connect(function()
	...
	LongJump = taskware.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			updateVelocity()
			if callback then
				LongJump:Clean(taskEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(taskEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	local ProjectileAimbot = taskware.Categories.Blatant:CreateModule({
...
run(function()
	...
	ProjectileAura = taskware.Categories.Blatant:CreateModule({
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedESP = taskware.Categories.Render:CreateModule({
		Name = 'BedESP',
		Function = function(callback)
			if callback then
	...
	Health = taskware.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Health:Clean(label)
				Health:Clean(taskEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	KitESP = taskware.Categories.Render:CreateModule({
		Name = 'KitESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	StorageESP = taskware.Categories.Render:CreateModule({
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = taskware.Categories.Utility:CreateModule({
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoKit = taskware.Categories.Utility:CreateModule({
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPlay = taskware.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(taskEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(taskEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	AutoToxic = taskware.Categories.Utility:CreateModule({
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(taskEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(taskEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(taskEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = taskware.Categories.Utility:CreateModule({
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = taskware.Categories.Utility:CreateModule({
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	PickupRange = taskware.Categories.Utility:CreateModule({
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
...
run(function()
	...
	RavenTP = taskware.Categories.Utility:CreateModule({
		Name = 'RavenTP',
		Function = function(callback)
			if callback then
				RavenTP:Toggle()
...
run(function()
	...
	Scaffold = taskware.Categories.Utility:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	ShopTierBypass = taskware.Categories.Utility:CreateModule({
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function staffFunction(plr, checktype)
		if not taskware.Loaded then repeat task.wait() until taskware.Loaded end
	...
	local function playerAdded(plr)
		...
		if getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and taskware.Loaded and Clans.Enabled then
				connection:Disconnect()
	...
	task.spawn(function()
		repeat task.wait(1) until taskware.Loaded or taskware.Loaded == nil
		if taskware.Loaded and not StaffDetector.Enabled then
...
run(function()
	StoreDamage = taskware.Categories.Utility:CreateModule({
		Name = 'StoreDamage',
...
run(function()
	TrapDisabler = taskware.Categories.Utility:CreateModule({
		Name = 'TrapDisabler',
...
run(function()
	...
	AutoTool = taskware.Categories.World:CreateModule({
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedProtector = taskware.Categories.World:CreateModule({
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChestSteal = taskware.Categories.World:CreateModule({
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = taskware.Categories.World:CreateModule({
		Name = 'Schematica',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ArmorSwitch = taskware.Categories.Inventory:CreateModule({
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = taskware.Categories.Inventory:CreateModule({
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				repeat task.wait() until store.queueType ~= 'bedwars_test'
				...
				local lastupgrades
				AutoBuy:Clean(taskEvents.InventoryAmountChanged.Event:Connect(function()
...
run(function()
	...
	AutoConsume = taskware.Categories.Inventory:CreateModule({
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
				AutoConsume:Clean(taskEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
				AutoConsume:Clean(taskEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		window.Parent = taskware.gui.ScaledGui
		...
		close.MouseButton1Click:Connect(function()
			window.Visible = false
			taskware.gui.ScaledGui.ClickGui.Visible = true
		end)
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / taskware.guiscale.Scale)
		end)
		table.insert(taskware.Windows, window)
	...
	taskware.Components.HotbarList = function(optionsettings, children, api)
		if taskware.ThreadFix then
			setthreadidentity(8)
		end
		local optionapi = {
		...
		textbuttonicon.Image = getcustomasset('taskware/assets/new/add.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / taskware.guiscale.Scale, 603))
		end)
		...
		function optionapi:AddHotbar(data)
			...
			hotbar.MouseButton1Click:Connect(function()
				...
				if ind == optionapi.Selected then
					taskware.gui.ScaledGui.ClickGui.Visible = false
	...
	AutoHotbar = taskware.Categories.Inventory:CreateModule({
		Name = 'AutoHotbar',
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(taskEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = taskware.Categories.Inventory:CreateModule({
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = taskware.Categories.Inventory:CreateModule({
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedPlates = taskware.Categories.Minigames:CreateModule({
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
				for _, v in collectionService:GetTagged('bed') do 
				...
				BedPlates:Clean(taskEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(taskEvents.BreakBlockEvent.Event:Connect(refreshNear))
...
run(function()
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('taskware/assets/new/blur.png'),
	...
	Breaker = taskware.Categories.Minigames:CreateModule({
		Name = 'Breaker',
		Function = function(callback)
			if callback then
				for _ = 1, 30 do
...
run(function()
	...
	local Crosshair = taskware.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	DamageIndicator = taskware.Legit:CreateModule({
...
run(function()
	...
	FPSBoost = taskware.Legit:CreateModule({
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = taskware.Legit:CreateModule({
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = taskware.Legit:CreateModule({
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = taskware.Legit:CreateModule({
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = taskware.Legit:CreateModule({
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SoundChanger = taskware.Legit:CreateModule({
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	UICleanup = taskware.Legit:CreateModule({
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do 
...
run(function()
	...
	Viewmodel = taskware.Legit:CreateModule({
		Name = 'Viewmodel',
		Function = function(callback)
...
run(function()
	...
	WinEffect = taskware.Legit:CreateModule({
	    Name = 'WinEffect',
	    Function = function(callback)
	        if callback then
	            WinEffect:Clean(taskEvents.MatchEndEvent.Event:Connect(function()
